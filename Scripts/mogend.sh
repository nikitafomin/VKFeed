...
#  for enabling this script you should go to "Project target" -> "Build Rules" -> "Editor" -> "Add build Rule" -> select "Data model version files using Script" -> Process = "Data model version files" -> add custom script
        #echo "Running mogend"
        #"${SRCROOT}/VKFeed/Scripts/mogend.sh"
...
mogenerator --model "${INPUT_FILE_PATH}/$curVer" --machine-dir "${PROJECT_DIR}/VKFeed/Classes/CoreData/Private/" --human-dir "${PROJECT_DIR}/VKFeed/Classes/CoreData/" --template-var arc=true
